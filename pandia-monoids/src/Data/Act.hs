...
newtype ActDeepMap s = ActDeepMap {unactDeepMap :: s}
  deriving stock (Show, Eq)
  deriving newtype (Semigroup, Monoid)

instance  (LAct x (ActDeepMap s), Functor f) => LAct (f x) (ActDeepMap s) where
  s <>$ x = fmap (s <>$) x
  {-# INLINE (<>$) #-}

instance {-# INCOHERENT #-} (LAct x s) => LAct x (ActDeepMap s) where
  ActDeepMap s <>$ x = s <>$ x
  {-# INLINE (<>$) #-}

(+$) :: LAct f (ActDeepMap (Sum a)) => a -> f -> f
a +$ f = ActDeepMap (Sum a) <>$ f
